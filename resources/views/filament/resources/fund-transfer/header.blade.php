<div class="mb-6 space-y-4">
    {{-- Floating Cash at Panitia --}}
    @if (isset($floatingCash) && $floatingCash > 0)
        <div
            class="fi-section rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 overflow-hidden">
            <div class="px-6 py-4 flex items-center justify-between">
                <div>
                    <div class="text-base font-semibold text-amber-800 dark:text-amber-200">
                        💰 Kas Mengendap di Panitia
                    </div>
                    <p class="text-sm text-amber-600 dark:text-amber-400">
                        Dana yang belum didistribusikan ke Unit
                    </p>
                </div>
                <div class="text-2xl font-bold text-amber-700 dark:text-amber-300">
                    Rp {{ number_format($floatingCash, 0, ',', '.') }}
                </div>
            </div>
        </div>
    @endif
                    ...
                    <div
                        ...
                        <div class="flex gap-2 mt-1 text-xs">
                            @if ($item['pending_count'] > 0)
                                <span
                                    class="px-2 py-0.5 rounded bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">{{ $item['pending_count'] }}
                                    Menunggu</span>
                            @endif
                            @if ($item['approved_count'] > 0)
                                <span
                                    class="px-2 py-0.5 rounded bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">{{ $item['approved_count'] }}
                                    Disetujui</span>
                            @endif
                        </div>
                    </div>
    ...
    {{-- Priority Algorithm Summary Table --}}
    <div
        ...
        <div class="fi-section-header flex flex-col gap-3 px-6 py-4">
            <div class="flex items-center gap-3">
                <div class="fi-section-header-heading text-base font-semibold leading-6 text-gray-950 dark:text-white">
                    📊 Hak Dana Per Lembaga (Priority Algorithm)
                </div>
            </div>
            <p class="fi-section-header-description text-sm text-gray-500 dark:text-gray-400">
                Perhitungan prioritas: Madrasah (100%) → 50:50 Sekolah/Pondok
            </p>
        </div>
                        ...
                        @foreach ($summary as $item)
                            <tr class="fi-ta-row transition duration-75 hover:bg-gray-50 dark:hover:bg-white/5">
                                ...
                                <td class="fi-ta-cell px-4 py-3 text-sm text-gray-500 dark:text-gray-400 text-end">
                                    Rp {{ number_format($item['total_entitlement'], 0, ',', '.') }}
                                </td>
                                <td class="fi-ta-cell px-4 py-3 text-sm text-gray-500 dark:text-gray-400 text-end">
                                    Rp {{ number_format($item['total_transferred'], 0, ',', '.') }}
                                </td>
